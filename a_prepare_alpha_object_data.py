...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 28 16:46:20 2025
...
for subject_num in range(1, 47):
    ...
    raw = mne.io.read_raw_fif(join(sample_path, 'wiica','clean', f"{subject}_ica_ob_raw.fif"), preload=True)
...
#labels    
    ...
    events = mne.merge_events(events, [12, 14], 14)
    events = mne.merge_events(events, [16, 18], 16)
    events = mne.merge_events(events, [11, 13, 15, 17], 11)
    events = mne.merge_events(events, [14, 16, 11], 30)
    ...
    for band_name, (freqs, n_cycles) in bands.items():
        ...
        # Baseline correction by subtraction in dB scale
        sensor_data_avg = sensor_data.mean(axis=2).astype(np.float32)   # average over freq 
